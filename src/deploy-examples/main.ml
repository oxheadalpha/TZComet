...
let all ?(dry_run = false) ?(print = true) ?only ~logfile () =
  ...
  let add name description kt1 =
    originated := (name, description, kt1) :: !originated
  in
    ...
    | None | Some _ ->
        let kt1 =
          ...
          | true ->
              ...
              Fmt.str "KT1FakeFakeooo%03d" (List.length !originated)
        in
        add name description kt1
  in
  let simple name description ?(the_nat = 7) bm =
    ...
    in
    originate ~description ~logfile ~name ~source ~init ()
  in
  let root uri = ("", uri) in
    ...
    view_with_code
      ...
      [
        ...
        prim "IF"
          [
            seq [ prim "FAILWITH" [] ];
            seq [ prim "PUSH" [ prim "int" []; int 2 ]; prim "MUL" [] ];
          ];
      ]
  in
  let identity_01 =
        ...
        [
          ...
          prim "pair"
            [
              ...
              prim "mutez" ~annotations:[ "%arg_two" ] [];
            ];
        ]
    in
    ...
    view_with_code ~pure:true ~parameter:nat "multiply-the-nat-in-storage" code
      ~description:
        "This one is pure, it multiplies the natural number given as argument \
         with the one in storage."
  in
  let call_balance =
    let code = prims [ "DROP"; "BALANCE" ] in
    view_with_code ~return_type:mutez "just-call-balance" code
  in
  ...
  let unit_to_bytes name value =
    let code =
      [ prim "DROP" []; prim "PUSH" [ prim "bytes" []; michbytes value ] ]
    in
    view_with_code name code
      ~return_type:(prim "bytes" [] ~annotations:[ "%returnedBytes" ])
      ~annotations:[ ("%returnedBytes", "A bytes constant.") ]
  in
...
let () =
  ...
  let dry_run =
    try String.equal (Caml.Sys.getenv "dryrun") "true" with _ -> false
  in
