    ...
    (let row l = H5.tr (List.map ~f:td l) in
             ...
             let extra_info =
               try
                 ...
                 let field l f =
                   ...
                   with _ -> Fmt.failwith "Missing field: %S" f
                 in
     ...
     let ping_date date =
       if Float.(date < 10.) then (* Construction sign: *) t "🚧"
         ...
         in
         Bootstrap.monospace (t date_string)
     in
     let row_of_node n =
       row
         Query_nodes.Node.
           [
             ...
             Bootstrap.button ~outline:true ~size:`Small ~kind:`Danger
               (t "Remove 💣") ~action:(fun () ->
                 Query_nodes.remove_node ctxt ~name:n.name);
           ]
     in
     let last_row =
         ...
         [
           ...
           Bootstrap.button (t "⇑ Ping'em'all") ~kind:`Secondary
             ~action:(fun () ->
               ...
               Query_nodes.Update_status_loop.wake_up ctxt);
         ]
     in
