...
(** Non-blocking streaming JSON codec.
    ...
    [Jsonm] is a non-blocking streaming codec to {{!section:decode} decode} and
    {{!section:encode} encode} the {{:http://tools.ietf.org/html/rfc7159} JSON}
    data format. It can process JSON text without blocking on IO and without a
    complete in-memory representation of the data.

    The {{!Uncut} uncut codec} also processes whitespace and (non-standard) JSON
    with JavaScript comments.

    Consult the {{!datamodel} data model}, {{!limitations} limitations} and
...
(** The type for JSON lexemes. [`As] and [`Ae] start and end arrays and [`Os]
    ...
    A {e well-formed} sequence of lexemes belongs to the language of the [json]
    grammar:
    ...
    A {{!section:decode} decoder} returns only well-formed sequences of lexemes
    or [`Error]s are returned. The {{:http://tools.ietf.org/html/rfc3629}
    UTF-8}, {{:http://tools.ietf.org/html/rfc2781} UTF-16}, UTF-16LE and
    UTF-16BE encoding schemes are supported. The strings of decoded [`Name] and
    [`String] lexemes are however always UTF-8 encoded. In these strings,
    characters originally escaped in the input are in their unescaped
    representation.

    An {{!section:encode} encoder} accepts only well-formed sequences of lexemes
    or [Invalid_argument] is raised. Only the UTF-8 encoding scheme is
    supported. The strings of encoded [`Name] and [`String] lexemes are assumed
    to be immutable and must be UTF-8 encoded, this is {b not} checked by the
...
(** [decode d] is:
    ...
    - [`Await] if [d] has a [`Manual] source and awaits for more input. The
      client must use {!Manual.src} to provide it.
    - [`Lexeme l] if a lexeme [l] was decoded.
    - [`End] if the end of input was reached.
    - [`Error e] if a decoding error occured. If the client is interested in a
      best-effort decoding it can still continue to decode after an error (see
      {!errorrecovery}) although the resulting sequence of [`Lexeme]s is
      undefined and may not be well-formed.
...
(** [decoded_range d] is the range of characters spanning the last [`Lexeme] or
    [`Error] (or [`White] or [`Comment] for an {!Uncut.decode}) decoded by [d].
...
(** [decoder_encoding d] is [d]'s encoding.
    ...
    {b Warning.} If the decoder guesses the encoding, rely on this value only
    after the first [`Lexeme] was decoded. *)
...
(** [encode e v] is:
    ...
    - [`Partial] iff [e] has a [`Manual] destination and needs more output
      storage. The client must use {!Manual.dst} to provide a new buffer and
      then call {!encode} with [`Await] until [`Ok] is returned.
    - [`Ok] when the encoder is ready to encode a new [`Lexeme] or [`End].

    For [`Manual] destinations, encoding [`End] always returns [`Partial], the
    client should as usual use {!Manual.dst} and continue with [`Await] until
    [`Ok] is returned at which point {!Manual.dst_rem} [e] is guaranteed to be
    the size of the last provided buffer (i.e. nothing was written).

    {b Raises.} [Invalid_argument] if a non {{!datamodel} well-formed} sequence
    of lexemes is encoded or if [`Lexeme] or [`End] is encoded after a
    [`Partial] encode. *)
...
(** Codec with comments and whitespace.
    ...
    The uncut codec also processes whitespace and JavaScript comments. The
    latter is non-standard JSON, fail on [`Comment] decoding if you want to
    process whitespace but stick to the standard.

    The uncut codec preserves as much of the original input as possible. Perfect
    round-trip with [Jsonm] is however impossible for the following reasons:

    - Escapes unescaped by the decoder may not be escaped or escaped differently
      by the encoder.
    - The encoder automatically inserts name separator [':'] and value
      separators [","]. If you just reencode the sequence of decodes, whitespace
      and comments may (harmlessly, but significantly) commute with these
      separators.
...
module Uncut : sig
  (** {1:uncutdatamodel Uncut data model}
      ...
      The uncut data model is the same as the regular {{!datamodel} data model},
      except that before or after any lexeme you may decode/encode one or more:

      - [`White w], representing JSON whitespace [w]. On input the sequence CR
        ([U+000D]) and CRLF (<[U+000A], [U+000A]>) are normalized to [U+000A].
        The string [w] must be a sequence of [U+0020], [U+0009], [U+000A] or
        [U+000D] characters ([' '], ['\t'], ['\n'], ['\r']).
      - [`Comment (`S, c)], representing a JavaScript single line comment [c].
        [c] is the comment's content without the starting [//] and the ending
        newline. The string [c] must not contain any newline.
      - [`Comment (`M, c)], representing a JavaScript multi-line comment [c].
        [c] is the comment's content without the starting [/*] and the ending
        [*/]. The string [c] must not contain the sequence [*/].

      {b Warning.} {!Uncut.encode} does not check the above constraints on [w]
      and [c]. *)
  ...
  (** [encode] is like {!Jsonm.encode} but for the {{!uncutdatamodel} uncut data
      ...
      {b IMPORTANT.} Never encode [`Comment] for the web, it is non-standard and
...
(** {1:limitations Limitations}
    ...
    - JSON numbers are represented with OCaml [float] values. This means that it
      can only represent integers exactly in the in the interval
      \[-2{^ 53};2{^ 53}\]. This is equivalent to the contraints JavaScript has.
    - A superset of JSON numbers is parsed. After having seen a minus or a
      digit, including zero, {!Stdlib.float_of_string}, is used. In particular
      this parses number with leading zeros, which are specifically prohibited
      by the standard.
    - Strings returned by [`String], [`Name], [`White] and [`Comment] are
      limited by {!Sys.max_string_length}. There is no built-in protection
      against the fact that the internal OCaml [Buffer.t] value may raise
      [Failure] on {!Jsonm.decode}. This should however only be a problem on
      32-bits platforms if your strings are greater than 16Mo.

    Position tracking assumes that each decoded Unicode scalar value has a
    column width of 1. The same assumption may not be made by the display
    program (e.g. for [emacs]' compilation mode you need to set
    [compilation-error-screen-columns] to [nil]).

    The newlines LF ([U+000A]), CR ([U+000D]), and CRLF are all normalized to LF
    internally. This may have an impact in some corner [`Error] cases. For
    example the invalid escape sequence [<U+005C,U+000D>] in a string will be
    reported as being [`Illegal_escape (`Not_esc_uchar
    0x000A)].

    {2 Encode}

    Encoders produce valid JSON provided the {e client} ensures that the
    following holds.
    ...
    - [`Float] lexemes must not be, {!Stdlib.nan}, {!Stdlib.infinity} or
      {!Stdlib.neg_infinity}. They are encoded with the format string ["%.16g"],
      this allows to roundtrip all the integers that can be precisely
      represented in OCaml [float] values, i.e. the integers in the interval
      \[-2{^ 53};2{^ 53}\]. This is equivalent to the constraints JavaScript
      has.
    - If the {{!Uncut} uncut} codec is used [`White] must be made of
      {{!Uncut.uncutdatamodel} JSON whitespace} and [`Comment] must never be
      encoded. *)
...
(** {1:errorrecovery Error recovery}
    ...
    - [`Illegal_bytes], [`Illegal_escape], [`Illegal_string_uchar], a Unicode
      {{:http://unicode.org/glossary/#replacement_character} replacement
      character} ([U+FFFD]) is substituted to the illegal sequence.
    - [`Illegal_literal], [`Illegal_number] the corresponding [`Lexeme] is
      skipped.
    - [`Expected r], input is discarded until a synchronyzing lexeme that
      depends on [r] is found.
...
(** {1:examples Examples}
    ...
    Using the [`Manual] interface, [trip_fd] does the same but between Unix file
    descriptors.

    {[
      let trip_fd ?encoding ?minify (fdi : Unix.file_descr)
          (fdo : Unix.file_descr) =
    ...
    The result of [memsel names src] is the list of string values of members of
    [src] that have their name in [names]. In this example, decoding errors are
    silently ignored.
    ...
    The result of [json_to_dst dst json] has the JSON text [json] written on
