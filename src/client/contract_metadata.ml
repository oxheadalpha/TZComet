                ...
                fun () ->
                  ...
                  | ok when ok = 200 || (ok = 206 && Option.is_some limit_bytes)
                    ->
                      ...
                      in
                      let as_string =
                        Js_of_ocaml.Typed_array.String.of_arrayBuffer res
                      in
...
module Content = struct
    ...
    try
      ...
      let contents =
        Json_encoding.destruct
          Tezai_contract_metadata.Metadata_contents.encoding jsonm
      in
    ...
    let encoding =
      ...
      let operator_transfer_policy =
        string_enum
          [
            ...
            ("owner-or-operator-transfer", Owner_or_operator_transfer);
          ]
      in
      let owner_transfer_policy =
        string_enum
          [
            ...
            ("required-owner-hook", Required_owner_hook);
          ]
      in
      let custom_permission_policy =
        conv
          ...
          (fun (tag, config_api) -> { tag; config_api })
          (obj2 (req "tag" string) (opt "config-api" string))
      in
      conv
  ...
  let find_michelson_view metadata ~view_name =
    ...
    List.find_map metadata.views ~f:(function
      | view when String.equal view.name view_name -> (
          ...
          |> function
          | Some v -> Some v
          | None -> Some (`No_michelson_implementation view))
      | _ -> None)
  ...
  let check_implementation_types ?check_parameter ~check_return impl =
    ...
    let param =
      ...
      | None, Some _ -> (`Missing_parameter, None)
    in
    let result =
  ...
  let validate_view ?check_parameter metadata ~view_name ~check_return =
    ...
    | Some (`Found (view, impl)) -> (
        let ( ((param_ok, _) as parameter_status),
          ...
          check_implementation_types impl ?check_parameter ~check_return
        in
        match (param_ok, result_ok) with
        | `Ok, `Ok -> Valid (view, impl)
        | _ ->
            Invalid
              { view; implementation = impl; parameter_status; return_status })

  let rec find_token_metadata_big_map ~storage_node ~type_node =
    let open Tezos_micheline.Micheline in
    let go (storage_node, type_node) =
      find_token_metadata_big_map ~storage_node ~type_node
    in
  ...
  let call_view_from_string (ctxt : _ Context.t) view ~address ~parameter_string
    ...
    let parameter =
      Tezai_michelson.Concrete_syntax.parse_exn ~check_indentation:false
        parameter_string ~check_primitives:false
    in
  ...
  let call_view_or_fail ctxt view ~parameter_string ~address ~log =
    let open Lwt.Infix in
    call_view_from_string ctxt view ~address ~parameter_string ~log >>= function
    | Ok o -> Lwt.return o
      ...
      if Option.is_none interface_claim (* && Option.is_none tokens *) then ()
        ...
        let check_nat =
          Michelson.Partial_type.Structure.(
            function Leaf { kind = Nat; _ } -> true | _ -> false)
        in
        let get_balance =
          let check_parameter =
            Michelson.Partial_type.Structure.(
          ...
          validate_view metadata ~view_name:"get_balance" ~check_parameter
            ~check_return:check_nat
        in
        let total_supply =
          validate_view metadata ~view_name:"total_supply"
            ~check_parameter:check_nat ~check_return:check_nat
        in
        let all_tokens =
          let check_return =
            Michelson.Partial_type.Structure.(
              function Leaf { kind = List Nat; _ } -> true | _ -> false)
          in
          validate_view metadata ~view_name:"all_tokens" ~check_return
        in
        let is_operator =
          let check_parameter =
            Michelson.Partial_type.Structure.(
          ...
          let check_return =
            Michelson.Partial_type.Structure.(
              function Leaf { kind = Bool; _ } -> true | _ -> false)
          in
        ...
        let token_metadata =
          ...
          validate_view metadata ~view_name:"token_metadata"
            ~check_parameter:check_nat ~check_return
        in
        let permissions_descriptor =
          match find_extra "permissions" with
          | None -> None
          | Some j -> Some (Permissions_descriptor.of_json j)
        in
        let metadata =
          {
            metadata with
      ...
      let find_remove l ~key =
        let rec go found acc = function
          ...
          | (kone, vone) :: more ->
              ...
              else go found ((kone, vone) :: acc) more
        in
        go None [] l
      in
              ...
              fun j ->
                let parse_one j =
                  ...
                  let get_string_field key =
                    get_string_field ~parent_name:"formats" d key
                  in
                  ...
                  let other =
                    List.filter d ~f:(fun (k, _) ->
                        not
                          (List.mem [ "uri"; "mimeType" ] ~equal:String.equal k))
                  in
                  { uri; mime_type; other }
                in
...
module Multimedia = struct
  ...
  let prepare_and_guess ~mime_types (ctxt : _ Context.t) ~uri ~log =
    ...
    let found ~format ?(sfw = false) ~converted uri =
      Lwt.return { uri; sfw; format; converted_uri = converted }
    in
    ...
    let guess_format content =
      ...
      | None ->
          ...
          failf "could not guess the format of the content: 0x%s"
            (bytes_summary ~left:24 ~right:0 hx)
    in
    let convert ~format uri =
      ...
      | Ok uri16, _ -> (
          ...
          | Some s, None ->
              ...
              fetch ~limit_bytes:128 ctxt uri16 ~log >>= fun content ->
              guess_format content >>= fun format -> Lwt.return (format, s)
          | None, _ ->
              Lwt.catch
                (fun () ->
                  fetch ctxt uri16 ~log >>= fun content ->
                  guess_format content >>= fun format ->
    ...
    in
    State.Metadata_metadata.sfw_multimedia ctxt uri >>= fun format ->
      ...
      | Prim (_, "Pair", [ _; full_map ], _) -> (
          ...
          let piece_of_metadata ?json_type key =
            piece_of_metadata ?json_type
              ...
              ~key ~metadata_map:key_values ~metadata_json
          in
          ...
          | Some (title, uri) ->
              ...
              >|= Option.some)
          >>= fun main_multimedia ->
          get_total_supply_with_view () >>= fun total_supply ->
